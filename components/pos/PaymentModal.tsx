...
type StkState = 'idle' | 'prompting' | 'sending' | 'waiting' | 'success' | 'failure';

const PaymentModal: React.FC<PaymentModalProps> = ({ cartItems, discount, onClose, onCompleteSale, customer, settings }) => {
    const [paymentType, setPaymentType] = useState<PaymentType>('M-Pesa');
    ...
    const PaymentButton: React.FC<{ method: PaymentType, children: React.ReactNode }> = ({ method, children }) => (
        <motion.button
                    ...
                    <>
                        ...
                        {paymentType === 'M-Pesa' && (
                             <div className="space-y-3">
                                ...
                                <button onClick={handleSendStkPush} disabled={stkState === 'sending' || stkState === 'waiting'} className="w-full bg-green-600 text-white font-semibold py-3 rounded-lg hover:bg-green-700 disabled:bg-slate-400">
                                    {stkState === 'sending' ? 'Sending...' : stkState === 'waiting' ? 'Waiting for PIN...' : 'Send STK Push'}
                                </button>
                            </div>
                        )}
