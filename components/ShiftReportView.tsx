        ...
        <table className="w-full text-sm text-left text-slate-500">
          ...
          <tbody>
            {closedShifts.map(shift => {
              ...
              return (
                <tr key={shift.id} className="bg-white border-b hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedShift(shift)}>
                  ...
                  <td className={`px-6 py-4 text-right font-mono font-semibold ${variance === 0 ? 'text-slate-700' : variance > 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {formatCurrency(variance)}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <span className="font-medium text-emerald-600 hover:underline">View Report</span>
                  </td>
                </tr>
              )
            })}
            {closedShifts.length === 0 && (
              <tr>
                <td colSpan={6} className="text-center py-10 text-slate-500">No closed shifts found.</td>
              </tr>
            )}
          </tbody>
        </table>
