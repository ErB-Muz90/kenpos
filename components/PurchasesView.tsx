...
const PurchasesView: React.FC<PurchasesViewProps> = ({ purchaseOrders, suppliers, products, onReceivePORequest, onAddPurchaseOrder, onAddSupplier, permissions }) => {
                    ...
                    <tbody>
                        {purchaseOrders.map(po => (
                            <tr key={po.id} className="bg-white border-b hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedPO(po)}>
                                ...
                                <td className="px-6 py-4">{supplierMap[po.id] || 'Unknown'}</td>
